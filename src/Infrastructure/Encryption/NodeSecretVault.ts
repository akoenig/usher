...
function encrypt(masterKey: Uint8Array, input: {
  readonly credentialId: CredentialId
  readonly purpose: string
  readonly plaintext: string
  ...
  return Effect.try({
    try: () => {
      ...
      const cipher = createCipheriv(Algorithm, key, nonce, { authTagLength: AuthenticationTagBytes })
...
function decrypt(masterKey: Uint8Array, input: {
  readonly credentialId: CredentialId
  readonly purpose: string
  readonly ciphertext: string
}): Effect.Effect<string, SemanticError> {
  return Effect.gen(function*() {
    ...
    return yield* Effect.try({
      try: () => {
        ...
        const key = deriveKey(masterKey, input.credentialId, input.purpose)
        const decipher = createDecipheriv(Algorithm, key, nonce, { authTagLength: AuthenticationTagBytes })
