...
import { CallerIpNotAllowedError, MissingUrlError, SemanticError, toErrorResponseBody, type SemanticError as SemanticErrorType } from "../../Domain/Errors/UsherErrors.js"
...
function admin(
  ...
  handler: () => Effect.Effect<
    ...
    unknown,
    HttpServerRequest.HttpServerRequest | CredentialService | OAuth2Service | HttpRouter.RouteContext | HttpServerRequest.ParsedSearchParams
  >
) {
...
function call(config: HttpServerConfig) {
  return Effect.gen(function*() {
    ...
    const response = yield* service.call({
    ...
    return response.body instanceof Uint8Array
      ? HttpServerResponse.uint8Array(response.body, { status: response.status, headers: response.headers })
      : HttpServerResponse.text(response.body, { status: response.status, headers: response.headers })
  })
}
