...
import { CredentialServiceLive, RedactedCredential } from "../../Application/Services/CredentialService.js"
...
describe("Usher E2E", () => {
  it.scoped("injects a BearerToken credential and returns the upstream response as-is", () =>
    Effect.gen(function*() {
      ...
      const result = yield* Effect.gen(function*() {
        ...
        const callResponse = yield* withInsecureLocalTls(HttpClientRequest.post(
  ...
  it.scoped("completes OAuth2 authorization, refreshes, injects access token, and returns upstream response as-is", () =>
    Effect.gen(function*() {
      const upstream = yield* startHttpsUpstream({
        status: 203,
      ...
      const result = yield* Effect.gen(function*() {
        ...
        const callbackResponse = yield* HttpClient.get(`/oauth2/callback?state=${encodeURIComponent(state)}&code=authorization-code`)
        const callbackBody = yield* callbackResponse.text
        const callResponse = yield* withInsecureLocalTls(HttpClientRequest.put(
      ...
      assert.strictEqual(refreshRequest?.body.includes("refresh_token=refresh-token-from-callback"), true)
...
function startHttpsUpstream(response: {
  ...
  return Effect.acquireRelease(
    Effect.async<FakeServer, unknown>((resume) => {
      ...
      const server = createHttpsServer({ key: LocalhostKey, cert: LocalhostCert }, (request, serverResponse) => {
      ...
      const server = createServer((request, serverResponse) => {
        collectRequest(request).then((body) => {
          ...
          const responseBody = body.includes("grant_type=authorization_code")
            ? { access_token: "access-token-from-code", refresh_token: "refresh-token-from-callback", scope: "calendar.readonly" }
...
const activeServers = new Map<string, ReturnType<typeof createServer> | ReturnType<typeof createHttpsServer>>()
...
const makeE2ELayer = Layer.unwrapScoped(Effect.gen(function*() {
  ...
  const vault = Layer.succeed(SecretVault, makeNodeSecretVault(new Uint8Array([
    0, 1, 2, 3, 4, 5, 6, 7,
