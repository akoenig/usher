...
import { NoMatchingCredentialError, UpstreamRequestFailedError } from "../../Domain/Errors/UsherErrors.js"
...
describe("HttpServer", () => {
  ...
  it.effect("allows OAuth2 login from non-loopback peers and uses configured base URL for redirect URI", () =>
    Effect.gen(function*() {
      const commands = yield* Ref.make<ReadonlyArray<CallCommand>>([])
      const oauthLogins = yield* Ref.make<ReadonlyArray<{ readonly credentialId: string; readonly redirectUri: string }>>([])
      ...
      return yield* Effect.gen(function*() {
        ...
        const response = yield* HttpClient.get("/credentials/cred_0123456789abcdef/oauth2/login", {
  ...
  it.effect("allows OAuth2 callback from non-loopback peers and uses configured base URL for redirect URI", () =>
    Effect.gen(function*() {
      const commands = yield* Ref.make<ReadonlyArray<CallCommand>>([])
      const oauthCallbacks = yield* Ref.make<ReadonlyArray<{ readonly state: string; readonly code: string; readonly redirectUri: string }>>([])
      ...
      return yield* Effect.gen(function*() {
        ...
        const response = yield* HttpClient.get("/oauth2/callback?state=oauth-state&code=authorization-code", {
          headers: { host: "attacker.example.test" }
        })
      ...
      }).pipe(Effect.scoped, Effect.provide(makeTestLayer(commands, "success", { oauthCallbacks })))
    }))
  ...
  it.effect("forwards call method body and headers and preserves upstream status and body", () =>
    Effect.gen(function*() {
      ...
      return yield* Effect.gen(function*() {
        yield* HttpServer.serveEffect(makeHttpApp({ allowedCallerIps: [], baseUrl: "https://usher.example.com" }))
...
function makeTestLayer(
  ...
  refs?: {
    readonly oauthLogins?: Ref.Ref<ReadonlyArray<{ readonly credentialId: string; readonly redirectUri: string }>>
    readonly oauthCallbacks?: Ref.Ref<ReadonlyArray<{ readonly state: string; readonly code: string; readonly redirectUri: string }>>
  }
) {
  ...
  return Layer.mergeAll(
    ...
    Layer.succeed(OAuth2Service, {
      buildLoginUrl: (input) => Effect.gen(function*() {
        if (oauthLogins !== undefined) {
          yield* Ref.update(oauthLogins, (existing) => [...existing, {
      ...
      handleCallback: (input) => Effect.gen(function*() {
        if (oauthCallbacks !== undefined) {
          yield* Ref.update(oauthCallbacks, (existing) => [...existing, {
            state: input.state,
            code: input.code,
            redirectUri: input.redirectUri
          }])
