...
export const OAuth2HttpClient = Layer.provide(
  Layer.effect(
    ...
    Effect.gen(function*() {
      ...
      return {
        buildAuthorizationUrl: (input) => Effect.sync(() => {
        ...
        exchangeAuthorizationCode: (input) => requestToken(httpClient, input.tokenUrl, {
        ...
        refreshAccessToken: (input) => requestToken(httpClient, input.tokenUrl, {
...
function requestToken(
  ...
  return Effect.gen(function*() {
    ...
    return yield* Schema.decodeUnknown(OAuth2TokenResponse)({
      ...
      scopes: decoded.scope === undefined ? undefined : decoded.scope.split(" ").filter((scope) => scope.length > 0)
