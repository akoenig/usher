...
import { HttpExecutor, type HeaderRecord, type OutboundBody, type UpstreamResponse } from "../Ports/HttpExecutor.js"
  ...
  Effect.gen(function*() {
    ...
    function recordOutcome(input: {
      ...
      return auditLog.record({
        ...
        ...(input.matchedCredentialId === undefined ? {} : { matchedCredentialId: input.matchedCredentialId }),
    ...
    function call(command: CallCommand) {
      return Effect.gen(function*() {
        ...
        const matches = credentials.filter((credential) =>
          ...
          credential.allowedRequests.some((allowedRequest) => allowedRequestMatches(allowedRequest, targetUrl))
        )
        ...
        const request = command.body === undefined ? {
...
function stripHopByHopHeaders(headers: HeaderRecord): HeaderRecord {
  const connectionHeaders = findHeaderValue(headers, "connection")
    ?.split(",")
