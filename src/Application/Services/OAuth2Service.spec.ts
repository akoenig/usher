...
function makeCredentialRepository(
  ...
  return {
    ...
    insertOAuthState: (state: OAuthState) => Ref.update(states, (oauthStates) => [...oauthStates, state]),
    consumeOAuthState: ({ state, now }: { readonly state: string; readonly now: string }) => Effect.gen(function*() {
      ...
      yield* Ref.update(states, (storedStates) => storedStates.filter((storedState) => storedState.state !== state))
...
function makeSecretVault() {
  return {
    ...
    decrypt: ({ ciphertext }: { readonly ciphertext: string }) => Effect.succeed(ciphertext.replace("encrypted:", ""))
  }
}
...
function makeOAuth2Client(
  ...
  return {
    ...
    exchangeAuthorizationCode: ({ code }: { readonly code: string }) => Effect.gen(function*() {
