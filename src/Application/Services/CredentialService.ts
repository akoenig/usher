...
const RedactedCredentialFields = {
  ...
  label: Schema.String,
  allowedRequests: Schema.NonEmptyArray(Schema.Struct({
    url: Schema.Struct({
...
export type RedactedBearerTokenCredential = Schema.Schema.Type<typeof RedactedBearerTokenCredential>
...
export class CredentialService extends Context.Tag("CredentialService")<
  CredentialService,
  {
    ...
    readonly getById: (credentialId: CredentialId) => Effect.Effect<RedactedCredential, SemanticError>
  ...
  return Layer.effect(
    ...
    Effect.gen(function*() {
      ...
      return {
        ...
        list: () => repository.list().pipe(Effect.map((credentials) => credentials.map(redactCredential(config.baseUrl)))),
        getById: (credentialId) => repository.getById(credentialId).pipe(
          Effect.map(redactCredential(config.baseUrl))
        ),
...
function createCredential(
  ...
  return Effect.gen(function*() {
    ...
    const credential = yield* Match.value(input).pipe(
      Match.when({ type: "BearerToken" }, (bearerInput) => Effect.gen(function*() {
      ...
      Match.when({ type: "OAuth2" }, (oauth2Input) => Effect.gen(function*() {
...
function hasOverlap(
  ...
  return allowedRequests.some((allowedRequest) => existingCredentials.some((credential) =>
